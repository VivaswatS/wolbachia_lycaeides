...
foreach my $fastq (@ARGV){
	if($fastq =~ m/^\S+\/([A-Za-z]{2,}\S+)\.fastq\.gz$/){
	    my $id = $1;
		my $ref = "$mybasedir"."genome/norm.super";
		my $sam = "$mybasedir"."matches/samfiles/Super/hitFiles_"."$id"."\.sam";
		my $aln = "$mybasedir"."matches/alignedfiles/Super/"."$id"."\.fastq";
	
		my $job = "bowtie2 -a -x "."$ref"." "."$fastq"." -p 8 -S "."$sam"." --no-unal --quiet --al "."$aln"."\n";
		push @jobarray, $job;
	}
}

## run for mtDNA alignments in Plebejus Argus sequence 
#my $mybasedir = "/project/evolgen/vshastry/wolbachia/";
#foreach my $fastq (@ARGV){
#	if($fastq =~ m/^\S+\/([A-Za-z]{2,}\S+)\.fastq\.gz$/){
#	    my $id = $1;
#		my $ref = "$mybasedir"."genome/argus";
#		my $sam = "$mybasedir"."matches/samfiles/Argus/hitFiles_"."$id"."\.sam";
#		my $aln = "$mybasedir"."matches/alignedfiles/Argus/"."$id"."\.fastq";
#	
#		my $job = "bowtie2 -a -x "."$ref"." "."$fastq"." -p 8 -S "."$sam"." --no-unal --quiet --al "."$aln"."\n";
#		push @jobarray, $job;
#	}
...
push @modules, 'module load bowtie2 samtools';
